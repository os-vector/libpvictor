...
void spine_write_frames()
{
    int handle = spine_handle;
...
void spine_read_frames()
{
    ...
    while (1)
    {
        ...
        if (parsed_frame_size)
        {
            ...
            else if (parsed_frame_type == 0x6b61) {
                memcpy(&spine_data, parsed_frame_data, parsed_frame_size);
            } else {
                printf("unknown parsed frame type");
            }
            parsed_frame_size = 0;
        }   
        break;
...
void spine_led_demo(int handle) {
    uint8_t led_data[] = {0x00,0xFF,0x00, 0xFF,0x00,0x00, 0xFF,0x00,0x00, 0x00,0xFF,0x00, 0x00,0x00,0x00,0x00};
    for(int i = 0; i < 16; i++) {
        printf("leds[%d]: 0x%02x\n", i, led_data[i]);
    }
...
void set_motor_states(int16_t m1, int16_t m2, int16_t m3, int16_t m4) {
    int16_t motors[4] = {m1*100, m2*100, m3*100, m4*100};
    spine_update(spine_handle, 8888, 0, motors, NULL);
}

void set_body_states(int16_t m1, int16_t m2, int16_t m3, int16_t m4, uint32_t led1, uint32_t led2, uint32_t led3) {
    uint8_t leds[16] = {
        (led1 >> 16) & 0xFF, (led1 >> 8) & 0xFF, led1 & 0xFF,
        (led2 >> 16) & 0xFF, (led2 >> 8) & 0xFF, led2 & 0xFF,
        (led3 >> 16) & 0xFF, (led3 >> 8) & 0xFF, led3 & 0xFF,
        (led3 >> 16) & 0xFF, (led3 >> 8) & 0xFF, led3 & 0xFF,
        0x00, 0x00, 0x00, 0x00
    };
    int16_t motors[4] = {m1*100, m2*100, m3*100, m4*100};
    spine_update(spine_handle, 8888, 0, motors, (uint32_t*)leds);
}

void set_led_states(uint32_t led1, uint32_t led2, uint32_t led3) {
    uint8_t leds[16] = {
        (led1 >> 16) & 0xFF, (led1 >> 8) & 0xFF, led1 & 0xFF,
        (led2 >> 16) & 0xFF, (led2 >> 8) & 0xFF, led2 & 0xFF,
        (led3 >> 16) & 0xFF, (led3 >> 8) & 0xFF, led3 & 0xFF,
    ...
    spine_set_led(spine_handle, (uint32_t*)leds);
}
